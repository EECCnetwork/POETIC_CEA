...
source("Markov_Model/probabilistic_analysis/Results/one_way/daly_weights/results_YLDsevere_nocriticalcare.R")

#load data for tz critical care

source("Markov_Model/probabilistic_analysis/Results/one_way/daly_weights/results_YLDsevere_tzcriticalcare.R")


#load data for tz critical care

#source("Markov_Model/probabilistic_analysis/Results/one_way/daly_weights/results_YLDsevere_iscriticalcare.R")

#rbind all three dataframes

final_dat <- rbind(no_oneway_dat, tz_oneway_dat)

daly_severe_plot <-ggplot(final_dat, aes(x = Pars,y = NHB_MEAN, colour=Int)) +
  ...
  scale_x_continuous(limits = c(0.08, 0.19), breaks = c(seq(0.08, 0.19, by = 0.01))) +
  theme_bw() +
  ggtitle("DALY weights") +
  ylab("Conditional Expected Net Health Benefits") +
  xlab("Disability weight for a severe care episode") +
