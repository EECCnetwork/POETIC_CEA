...
source("Markov_Model/probabilistic_analysis/Results/one_way/critical_probabilities/results_crittodeath_nocriticalcare.R")

#load data for tz critical care

source("Markov_Model/probabilistic_analysis/Results/one_way/critical_probabilities/results_crittodeath_tzcriticalcare.R")


#load data for tz critical care

#source("Markov_Model/probabilistic_analysis/Results/one_way/critical_probabilities/results_crittodeath_iscriticalcare.R")

#rbind all three dataframes

final_dat <- rbind(no_oneway_dat, tz_oneway_dat)

plot_crittodeath <-ggplot(final_dat, aes(x = Pars,y = NHB_MEAN, colour=Int)) +
