...
source("Markov_Model/probabilistic_analysis/Results/one_way/critical_probabilities/results_crittosev_nocriticalcare.R")

#load data for tz critical care

source("Markov_Model/probabilistic_analysis/Results/one_way/critical_probabilities/results_crittosev_tzcriticalcare.R")


#load data for tz critical care

#source("Markov_Model/probabilistic_analysis/Results/one_way/critical_probabilities/results_crittosev_iscriticalcare.R")

#rbind all three dataframes

final_dat <- rbind(no_oneway_dat, tz_oneway_dat)

plot_crittosev <-ggplot(final_dat, aes(x = Pars,y = NHB_MEAN, colour=Int)) +
