...
source("Markov_Model/probabilistic_analysis/Results/one_way/aos_effectiveness/results_crittodeath_aoseff_nocriticalcare.R")

#load data for tz critical care

source("Markov_Model/probabilistic_analysis/Results/one_way/aos_effectiveness/results_crittodeath_aoseff_tzcriticalcare.R")


#load data for tz critical care

#source("Markov_Model/probabilistic_analysis/Results/one_way/aos_effectiveness/results_crittodeath_aoseff_iscriticalcare.R")

#rbind all three dataframes

final_dat <- rbind(no_oneway_dat, tz_oneway_dat)

triage_plot_crittodeath_aoseff <-ggplot(final_dat, aes(x = Pars,y = NHB_MEAN, colour=Int)) +
  ...
  xlab("Effectiveness of AOS in reducing probability of critical to death (%)") +
