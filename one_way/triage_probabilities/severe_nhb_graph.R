...
source("Markov_Model/probabilistic_analysis/Results/one_way/triage_probabilities/results_probsevere_nocriticalcare.R")

#load data for tz critical care

source("Markov_Model/probabilistic_analysis/Results/one_way/triage_probabilities/results_probsevere_tzcriticalcare.R")


#load data for tz critical care

#source("Markov_Model/probabilistic_analysis/Results/one_way/triage_probabilities/results_probsevere_iscriticalcare.R")

#rbind all three dataframes

final_dat <- rbind(no_oneway_dat, tz_oneway_dat)

triage_plot_severe <-ggplot(final_dat, aes(x = Pars,y = NHB_MEAN, colour=Int)) +
  ...
  scale_x_continuous(limits = c(0.62, 0.82), breaks = c(seq(0.64, 0.82, by = 0.02))) +
  theme_bw() +
