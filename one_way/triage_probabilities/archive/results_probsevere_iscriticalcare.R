...
source("Markov_Model/probabilistic_analysis/Results/one_way/triage_probabilities/pars_probsevere_iscriticalcare.R")
...
isaric_EECC_NHB <- one_way_nhb(parameter = isaric_oneway$params,
                             ...
                             wtp = 101,
                             intname = "Only EECC",
                             comp = "Regional or Referral Critical Care")
...
triage_plot_severe <-ggplot(is_oneway_dat, aes(x = Pars,y = NHB_MEAN, colour=Int)) + 
  geom_point(size = 3) +
  geom_line() +
  scale_colour_manual(values = c("Only EECC" = "#39568CFF", 
                                 "EECC & AOS" = "#73D055FF")) +
  geom_hline(yintercept=0, size = 0.5, linetype="dotted") +
  scale_x_continuous(limits = c(0.64, 0.8), breaks = c(seq(0.64, 0.8, by = 0.02))) +
  ...
  theme(legend.position = "right",
        ...
        panel.grid.miisr = element_blank(),
