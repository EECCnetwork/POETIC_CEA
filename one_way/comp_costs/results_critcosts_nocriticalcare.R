...
source("Markov_Model/probabilistic_analysis/Results/one_way/comp_costs/pars_critcosts_nocriticalcare.R")
...
nothing_oneway <- one_way_sense(outcome_dat = nothing_DALY_dat, 
                                 ...
                                 parameter = nothing_critical_costs)
...
plot_criticalcosts_nothing <-ggplot(no_oneway_dat, aes(x = Pars,y = NHB_MEAN, colour=Int)) +
  ...
  xlab("Unit Cost of Treating a Critical Patient in a no critical care setting ($)") +
