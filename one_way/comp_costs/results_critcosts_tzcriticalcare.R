...
source("Markov_Model/probabilistic_analysis/Results/one_way/comp_costs/pars_critcosts_tzcriticalcare.R")
...
tanzania_oneway <- one_way_sense(outcome_dat = tanzania_DALY_dat, 
                                 ...
                                 parameter = tanzania_critical_costs)
...
plot_criticalcosts_tanzania <-ggplot(tz_oneway_dat, aes(x = Pars,y = NHB_MEAN, colour=Int)) +
  ...
  scale_x_continuous(limits = c(0, 60), breaks = c(seq(0, 60, by = 10))) +
  theme_bw() +
  ggtitle("Comparator Costs") +
  ylab("Conditional Expected Net Health Benefits") +
  xlab("Unit Cost of Treating a Critical Patient in a District Level Hospital ($)") +
