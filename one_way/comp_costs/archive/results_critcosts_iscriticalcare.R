...
source("Markov_Model/probabilistic_analysis/Results/one_way/comp_costs/pars_critcosts_iscriticalcare.R")
...
is_eecc_costs <- costing_calc(outcome_dataset = is_eecc_sims,
                              severe_cost = isaric_severe_costs + eecc_severe_costs,
                              critical_cost = isaric_critical_costs + eecc_critical_costs,
                              death_cost = isaric_death_costs + eecc_death_costs,
                              discharge_cost = isaric_discharge_costs + eecc_discharge_costs)
...
is_eeccaos_costs <- costing_calc(outcome_dataset = is_eeccaos_sims,
                              severe_cost = isaric_severe_costs + eeccaos_severe_costs,
                              critical_cost = isaric_critical_costs + eeccaos_critical_costs,
                              death_cost = isaric_death_costs + eeccaos_death_costs,
                              discharge_cost = isaric_discharge_costs + eeccaos_discharge_costs)
...
isaric_oneway <- one_way_sense(outcome_dat = isaric_DALY_dat, 
                                 ...
                                 parameter = isaric_critical_costs)
...
plot_criticalcosts_isaric <-ggplot(is_oneway_dat, aes(x = Pars,y = NHB_MEAN, colour=Int)) +
  geom_point(aes(shape = Comp), size = 3) + 
  scale_colour_manual(values = c("Only EECC" = "#39568CFF", 
                                 "EECC & AOS" = "#73D055FF")) +
  scale_shape_manual(values=c("Regional or Referral Critical Care" = 9)) +
  geom_hline(yintercept=0, size = 0.5, linetype="dotted") +
  scale_x_continuous(limits = c(450, 1400), breaks = c(seq(450, 1400, by = 50))) +
  theme_minimal() +
  ggtitle("Comparator Costs") +
  ylab("Conditional Expected Net Health Benefits") +
  xlab("Unit Cost of Treating a Critical Patient in a Regional or Referral Hospital ($)") +
