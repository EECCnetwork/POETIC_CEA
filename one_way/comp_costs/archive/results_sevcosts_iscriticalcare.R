...
source("Markov_Model/probabilistic_analysis/Results/one_way/comp_costs/pars_sevcosts_iscriticalcare.R")
...
is_eecc_costs <- costing_calc(outcome_dataset = is_eecc_sims,
                              ...
                              discharge_cost = isaric_death_costs + eecc_discharge_costs)
...
is_eeccaos_costs <- costing_calc(outcome_dataset = is_eeccaos_sims,
                              ...
                              discharge_cost = isaric_death_costs + eeccaos_discharge_costs)
...
plot_severecosts_isaric <-ggplot(is_oneway_dat, aes(x = Pars,y = NHB_MEAN, colour=Int)) +
  ...
  scale_x_continuous(limits = c(180, 480), breaks = c(seq(180, 480, by = 20))) +
  theme_minimal() +
  ggtitle("Comparator Costs") +
  ylab("Conditional Expected Net Health Benefits") +
  xlab("Unit Cost of Treating a Severe Patient in a Regional or Referral Hospital ($)") +
