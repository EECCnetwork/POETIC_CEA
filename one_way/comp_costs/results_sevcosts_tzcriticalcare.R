...
source("Markov_Model/probabilistic_analysis/Results/one_way/comp_costs/pars_sevcosts_tzcriticalcare.R")
...
tanzania_oneway <- one_way_sense(outcome_dat = tanzania_DALY_dat, 
                                 ...
                                 parameter = tanzania_severe_costs)
...
plot_severecosts_tanzania <-ggplot(tz_oneway_dat, aes(x = Pars,y = NHB_MEAN, colour=Int)) +
  ...
  xlab("Unit Cost of Treating a Severe Patient in a District Level Hospital ($)") +
