...
source("Markov_Model/probabilistic_analysis/Results/one_way/int_costs/pars_eeccaoscritcosts_iscriticalcare.R")
...
isaric_oneway <- one_way_sense(outcome_dat = isaric_DALY_dat, 
                                 ...
                                 parameter = eeccaos_critical_costs)
...
plot_eeccaoscriticalcosts_isaric <-ggplot(is_oneway_dat, aes(x = Pars,y = NHB_MEAN, colour=Int)) +
  ...
  scale_x_continuous(limits = c(550, 2150), breaks = c(seq(550, 2150, by = 50))) +
  theme_minimal() +
  ggtitle("EECC & AOS Costs") +
  ylab("Conditional Expected Net Health Benefits") +
  xlab("Unit Cost of Treating a Critical Patient\nwith EECC & AOS in a Regional or Referral Hospital ($)") +
