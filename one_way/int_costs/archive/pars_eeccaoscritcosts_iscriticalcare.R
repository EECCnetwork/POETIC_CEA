...
eeccaos_critical_costs <- c(rep(550,nyear),rep(600,nyear),rep(650,nyear),rep(700,nyear),
                          rep(750,nyear),rep(800,nyear),rep(850,nyear),rep(900,nyear),
                          rep(950,nyear),rep(1000,nyear),rep(1050,nyear),rep(1100,nyear),
                         rep(1150,nyear),rep(1200,nyear),rep(1250,nyear),rep(1300,nyear),
                         rep(1350,nyear),rep(1400,nyear),
                         rep(1450,nyear),rep(1500,nyear),rep(1550,nyear),rep(1600,nyear),
                         rep(1650,nyear),rep(1700,nyear),rep(1750,nyear),rep(1800,nyear),
                         rep(1850,nyear),rep(1900,nyear),
                         rep(1950,nyear),rep(2000,nyear),rep(2050,nyear),
                         rep(2100,nyear),rep(2150,nyear))

sens_n <- length(eeccaos_critical_costs)
...
isaric_critical_costs <- rtri(n = sens_n, mod = 1327.46, min = 474.35, max = 1345.35)
...
eecc_severe_costs <- rtri(n = sens_n, mod = 217.20, min = 195.95, max = 492.60)
eecc_critical_costs <- rtri(n = sens_n, mod = 1368.27, min =502.97, max = 1397.35)
eecc_death_costs <- rep(0,sens_n)
eecc_discharge_costs <- rep(0,sens_n)

#EECC + AOS cost parameters

eeccaos_severe_costs <- rtri(n = sens_n, mod = 217.20, min = 195.95, max = 492.60)
