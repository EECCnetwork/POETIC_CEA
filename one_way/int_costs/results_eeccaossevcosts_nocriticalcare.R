...
source("Markov_Model/probabilistic_analysis/Results/one_way/int_costs/pars_eeccaossevcosts_nocriticalcare.R")
...
plot_eeccaosseverecosts_nothing <-ggplot(no_oneway_dat, aes(x = Pars,y = NHB_MEAN, colour=Int)) +
  ...
  scale_x_continuous(limits = c(10, 65), breaks = c(seq(10, 65, by = 5))) +
  theme_bw() +
  ggtitle("EECC & AOS Costs") +
  ylab("Conditional Expected Net Health Benefits") +
  xlab("Unit Cost of Treating a Severe Patient with\nEECC & AOS in a no critical care setting ($)") +
