...
source("Markov_Model/probabilistic_analysis/Results/one_way/int_costs/pars_eecccritcosts_tzcriticalcare.R")
...
plot_eecccriticalcosts_tanzania <-ggplot(tz_oneway_dat, aes(x = Pars,y = NHB_MEAN, colour=Int)) +
  ...
  scale_x_continuous(limits = c(0, 110), breaks = c(seq(0, 110, by = 10))) +
  theme_bw() +
  ggtitle("EECC Costs") +
  ylab("Conditional Expected Net Health Benefits") +
  xlab("Unit Cost of Treating a Critical Patient\nwith EECC in a District Level Hospital ($)") +
